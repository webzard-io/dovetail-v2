...
export const ConfigmapForm: React.FC<FormProps> = () => {
  const { formProps, saveButtonProps, editorProps, enableEditor, switchEditor } =
    useEagleForm({
      editorOptions: {
  ...
  return (
    <kit.form {...formProps} onFinish={formProps.onFinish} layout="horizontal">
      {enableEditor ? (
        editorProps.schema ? (
          <kit.form.Item>
            <YamlEditorComponent {...editorProps} schema={editorProps.schema} />
          </kit.form.Item>
        ) : (
