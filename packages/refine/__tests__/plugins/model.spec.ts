...
import { PodModel, WorkloadModel } from '../../src/models';
import { modelPlugin } from '../../src/plugins/model-plugin';
...
describe('PodModel', () => {
  ...
  it('format Pod', async () => {
    const podList = await modelPlugin.processData(RawPodList as UnstructuredList);
  ...
  it('deploymentModel can scale', async () => {
    ...
    expect((deploymentModel._rawYaml.spec as DeploymentSpec).replicas).toEqual(1);
  });
});
